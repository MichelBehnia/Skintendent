                                    ...
                                    <div className="animate-jump animate-duration-[1500ms] animate-delay-750 animate-ease-out">
                                        <a href={product.product_url} target="_blank" rel="noreferrer">
                                            <div className="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
                                                <img src={product.product_photo} alt="Product" className="h-80 w-72 object-cover rounded-t-xl" />
                                                <div className="px-4 py-3 w-72">
                                                    <span className="text-gray-400 mr-3 uppercase text-xs">{product.product_num_ratings} Reviews | {product.product_star_rating}/5</span>
                                                    <p key={product.title} className="text-lg font-bold text-black truncate block capitalize">{product.product_title}</p>
                                                    <div className="flex items-center">
                                                        <p className="text-lg font-semibold text-black cursor-auto my-3">${product.product_price}</p>
                                                        {
                                                            product.product_original_price ? <del><p class="text-sm text-gray-600 cursor-auto ml-2">${product.product_original_price}</p></del> : null
                                                        }
                                                    </div>
                                                </div>
                                            </div>
